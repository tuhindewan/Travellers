  ...
  <div class="panel-body">
    <div class="single_post_top">
      ...
      @foreach($getUserInfo as $userInfo)
        @if($userInfo['_id']==$post['fk_user_id']) 
        <?php
        $image_id = $userInfo['profile_image'];
        $image = App\Models\UserProfileImage::where('_id',$image_id)->first();
        $image = $image['image'];
        ?>
      ...
      @if($user['_id'] == $post['fk_user_id'])
        <a href="">{{$user['firstname']}} {{$user['middlename']}} {{$user['lastname']}}</a>
      @endif  
      @endforeach
        <br>
